...
Num = (1i*xc.^3).*(h1xs-(xs.*h1pxs));

Den = 3*((4*h1xs)+(2*xs.*h1pxs));

T1CCr = -(Num./Den);  %rigit particle 

Num2 = xc;

Den2 = (4*h1xs)+(2*xs.*h1pxs);
...
plot(real(xs), real(T0CCr./xc.^3), 'k','MarkerIndices',1:5:length(real(xs)),...
...
plot(real(xs), imag(T0CCr./xc.^3), 'm','MarkerIndices',1:5:length(real(xs)),...
