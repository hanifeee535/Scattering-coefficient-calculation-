...
%figure('NumberTitle','on', 'Name','T_0^CC');
    ...
    'MarkerSize',8,'MarkerEdgeColor','magenta','LineWidth',lw); % 
    %box on1
    %xlabel('\Ree(k_{s}a)',  'FontWeight','Bold', 'FontSize',fs);
    %ylabel('\Ree(T_{0}^{CC})', 'FontWeight','Bold', 'FontSize',fs);
    %set(gca, 'XScale','log', 'YScale','log', 'FontSize',fs); grid off;
    %set(gca, 'XTick',XT, 'XLim',[min(XT) max(XT)]);
     ...
     'MarkerSize',8,'MarkerEdgeColor','[0 0.5 0]','LineWidth',lw); 
     %set(gca, 'XScale','log', 'YScale','log', 'FontSize',fs); grid off;
     %ylabel('\Imm(T_{0}^{CC})', 'FontWeight','Bold', 'FontSize',fs);
     %leg1=legend('\Ree(T_{0e}^{CC})','\Ree(T_{0e}^{CC})','\Imm(T_{0r}^{CC})','\Imm(T_{0r}^{CC})');
     %set(leg1,'Location','NorthEast','FontSize',15,'FontWeight','Bold');
     %legend box off
     %hold off
